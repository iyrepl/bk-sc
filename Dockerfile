...
WORKDIR /root
RUN apk update
RUN apk add --no-cache ca-certificates iotop libgcc libc-dev libcurl libc-utils pcre-dev zlib-dev libnfs pcre pcre2 zip net-tools pstree libevent libevent-dev iproute2 wget git unzip gcc make supervisor tor sudo
#RUN apt update
